...
toggleMenu.onclick = () => {
  ...
  headerContainer.classList.add("container-full-height");
  document.body.style.overflow = "hidden";
  headerWrapper.style.overflow = "overlay";
};
toggleClose.onclick = () => {
  headerWrapper.classList.remove("menu-active");
  headerContainer.classList.remove("container-full-height");
...
var preSlide = () => {
  if (slideIndex === 0) {
    slideContainer.style.transform = `translateX(-${
      slideItem[slideIndex].offsetWidth * (slideItem.length - 1)
    }px)`;
    slideIndex = slideItem.length - 1;
  } else {
    slideContainer.style.transform = `translateX(-${
      slideItem[slideIndex].offsetWidth * (slideIndex - 1)
    }px)`;
    slideIndex--;
  }
};
